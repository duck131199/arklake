...
export type GatewayReadOnlyContextResult =
  | { ok: true; accountId: string; sessionId: string; userToken: string; circleUserId: string; arcWallet: StoredWallet }
...
export async function getGatewayReadOnlyContext(cookieHeader: string | undefined): Promise<GatewayReadOnlyContextResult> {
  ...
  return { ok: true, accountId: session.account_id, sessionId: session.sid, userToken: session.circle_user_token, circleUserId, arcWallet }
}
