alter table public.invoice_email_outbox
  drop constraint if exists invoice_email_outbox_event_type_check;

alter table public.invoice_email_outbox
  add constraint invoice_email_outbox_event_type_check
  check (event_type in ('invoice_created', 'invoice_paid', 'invoice_payment_confirmed'));
...
begin
  ...
  from public.arklake_accounts
  where id = target.account_id;

  if position('@' in coalesce(seller_email, '')) > 1 then
    insert into public.invoice_email_outbox (account_id, invoice_id, event_type, recipient_email)
    values (target.account_id, target.id, 'invoice_paid', seller_email)
    on conflict (invoice_id, event_type) do nothing;
  end if;

  if position('@' in coalesce(target.payer_email, '')) > 1 then
    insert into public.invoice_email_outbox (account_id, invoice_id, event_type, recipient_email)
    values (target.account_id, target.id, 'invoice_payment_confirmed', target.payer_email)
    on conflict (invoice_id, event_type) do nothing;
  end if;
