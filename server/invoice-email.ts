...
export async function renderInvoiceEmail(eventType: InvoiceEmailEvent, invoice: InvoiceEmailRecord, sellerEmail: string, recipientEmail: string, publicUrl: string) {
  ...
  const action = isPaid
    ...
    : `<p style="margin:26px 0 0"><a href="${escapeHtml(invoiceUrl)}" style="display:inline-block;border-radius:999px;background:#102a43;padding:13px 22px;color:#fff;font-size:14px;font-weight:700;text-decoration:none">View &amp; pay invoice</a></p><p style="margin:22px 0 0;color:#708593;font-size:12px;line-height:19px">Review the invoice details on Arklake before completing your payment.</p>`
