...
export function activityEmail(activity: EmailActivity) {
  ...
  const swapFromAmount = outgoing?.symbol ? `${outgoing.amount} ${outgoing.symbol}` : 'Amount unavailable'
  const swapToAmount = incoming?.symbol ? `${incoming.amount} ${incoming.symbol}` : 'Amount unavailable'
  const subject = activity.type === 'receive' ? `You received ${receivedAmount}`
    : activity.type === 'send' ? `${outgoingAmount} sent successfully` : `Swap confirmed: ${swapFromAmount} → ${swapToAmount}`
  const headline = activity.type === 'swap' ? 'Swap confirmed'
    : activity.type === 'send' ? `${outgoing?.symbol || 'Transfer'} sent successfully` : subject
  const supporting = activity.type === 'receive' ? 'Your transfer has been confirmed.'
    : activity.type === 'send' ? `Your transfer of ${outgoingAmount} has been confirmed.` : 'Your swap has been confirmed on-chain.'
  ...
  const details = activity.type === 'swap'
    ? [['You swapped', swapFromAmount], ['You received', swapToAmount], ['Confirmed at', time], ['Network', network], ...(activity.txHash ? [['Transaction', short(activity.txHash)]] : [])]
  ...
  const rows = details.map(([label, value]) => detailRow(label, value, activity.type === 'swap' && label === 'You received')).join('')
  const hero = activity.type === 'swap' ? `<div style="margin:25px 0 0;border-radius:14px;background:#f0f9f8;padding:17px 20px;text-align:center;color:#075e68;font-size:23px;font-weight:700;line-height:31px">${escapeHtml(swapFromAmount)} <span style="color:#61a7a9">→</span> ${escapeHtml(swapToAmount)}</div>` : ''
  const ctaLabel = activity.type === 'receive' || activity.type === 'swap' ? 'View on Arcscan' : 'View transaction'
