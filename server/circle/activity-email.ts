...
export function activityEmail(activity: EmailActivity) {
  ...
  const subject = activity.type === 'receive' ? `You received ${incomingAmount}`
  ...
  const details = activity.type === 'swap'
    ...
    : [['Amount', activity.type === 'receive' ? incomingAmount : outgoingAmount], ...(activity.type === 'receive' && from ? [['From', short(from)]] : []), ...(activity.type === 'send' && to ? [['To', short(to)]] : []), ['Time', time], ...(activity.type === 'send' && activity.networkFee ? [['Fee', activity.networkFee]] : []), ...(activity.txHash ? [['Transaction', short(activity.txHash)]] : [])]
