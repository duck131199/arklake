...
- EMAIL 3 — Invoice Payment Confirmed is CODE, MIGRATION, VISUAL, and LOCAL RUNTIME PASS; Production runtime has not been tested. The strict atomic Paid transition independently enqueues one seller `invoice_paid` and one payer `invoice_payment_confirmed`, with per-audience missing email isolation and stable `(invoice_id, event_type)` idempotency. Exact invoice Payment Intent transaction hashes suppress both generic Send and Receive email at enqueue and again before delivery; suppression is CODE/TEST PASS. The local invoice runtime test did not produce a `wallet_activity` for the transaction, but no generic Send or Receive email was sent. Unrelated transaction notifications are unchanged.
- Transaction EMAIL 3 — Receive is CODE, VISUAL, and LOCAL RUNTIME PASS; Production has not been tested. It renders the confirmed amount and known asset, sender, confirmed time, Arc Testnet network, shortened transaction hash, and Arcscan proof without exposing the destination wallet or inventing missing token metadata. Existing activity outbox, retry, idempotency, exact invoice-payment suppression, and feature-flag behavior are unchanged.
...
- cirBTC route-error UX is CODE PASS, TEST PASS, and LOCAL VISUAL/RUNTIME PASS; Production has not been tested. Circle `INPUT_UNSUPPORTED_ROUTE` / `1003` quotes now report that no route is currently available for the selected pair and amount without implying that a smaller amount will necessarily work.
- cirBTC Swap classification and email are CODE PASS, TEST PASS, and LOCAL RUNTIME PASS; Production has not been tested. Runtime transaction `0x07661abff9b18c07f9262558b9332aec8de7c995804b7c673b60e8bb7f396615` normalized `0.00001 cirBTC` to `3.687432 USDC` as exactly one Swap activity with two canonical legs, one outbox job, and one provider message, with no generic Send or Receive email. Arc receipt reconstruction uses canonical Arc Testnet USDC, EURC, and cirBTC addresses/decimals even when Circle balance/history metadata omits a fully spent token. Pure token receives remain Receive.
...
- Wallet Send + Receive email internal reconciliation is CODE PASS, MIGRATION PASS, LOCAL RUNTIME PASS, and duplicate-protection RUNTIME PASS; one side syncing a verified internal Arc Testnet USDC transfer materializes and delivers both account-scoped activities/outboxes, and the counterparty's later Circle-native sync replaces the reconciled leg without duplicate activity, job, or email. Production has not been tested.
- Wallet realtime Send/Receive balance and Recent Activity are CODE, TEST, MIGRATION, and LOCAL RUNTIME PASS; Production has not been tested. The root issue was that Circle balance/history indexing and account-scoped sync could add minutes of delay even after Arc confirmation. The shared balance refresh now reads canonical Arc USDC, while a persisted per-wallet Arc block cursor scans verified Transfer logs every five seconds on a visible Wallet page, overlaps recent blocks, and deduplicates by wallet, chain, and transaction hash. Runtime tests showed balance updates in about 1–10 seconds and Send/Receive Activity in the intended 5–15-second window without F5; focus/visibility refresh and in-flight guards remain in place.
- Arc-backed transaction email is CODE, TEST, and LOCAL RUNTIME PASS; Production has not been tested. A confirmed Arc receipt is persisted before its notification is enqueued and processed, so Send/Receive email no longer waits for Circle history indexing. The same activation cutoff, exact invoice-payment suppression, account isolation, outbox uniqueness, retry/backoff, and Resend idempotency guards run on the Arc path; a later Circle sync reconciles the same activity/job without duplicate email. Arc native USDC Receive logs at `0xfffffffffffffffffffffffffffffffffffffffe` are decoded as 18-decimal USDC for Activity without changing canonical invoice USDC at `0x3600000000000000000000000000000000000000`.
...
- Pay with Arklake + plaintext Memo V2 integration is **LOCAL RUNTIME PASS after the production-safe import fix** and has not been re-tested on production `arklake.site`. Local invoice `ARK-20260912-42FBD2FE` paid 1 USDC through the Circle UCW atomic V2 flow in transaction `0x3ee3133cf942de8eac7533beee23d240d0b53d075cfbb6018038f58f456f4865`; strict verification matched the canonical USDC payer, recipient, token and amount plus the V2 `InvoicePayment` event with exact `paymentReference = ARK-20260912-42FBD2FE` and plaintext `memo = Thank Duck` before the atomic Paid transition. Arcscan InvoicePayment decoding PASS. Local smoke checks also confirm that Pay with Arklake, Connect Wallet, and Scan to Pay can all create their shared Payment Intent after the import fix; production re-test remains pending.
- Connect Wallet + plaintext Memo V2 is **LOCAL RUNTIME PASS**; production has not been tested. Local invoice `ARK-20260913-ABD2706E` paid 1 USDC in transaction `0x57cf48993390f7df2b06c50a29319c0677b0d24b87dc22119163c4fd5466ea0f`. The strict verifier matched the external payer, recipient, canonical USDC token, amount, exact `paymentReference = ARK-20260913-ABD2706E`, and plaintext `memo = Axis` from the V2 `InvoicePayment` event before the invoice became Paid.
- Scan to Pay + plaintext Memo V2 is **LOCAL RUNTIME PASS**; production has not been tested. WalletConnect invoice `ARK-20260913-A1E97613` paid 1 USDC in transaction `0xde69e0988164e573741b5a90e09e6a33e7f7c6e3657be13126e65a414d69e5df`. The strict verifier matched the external payer `0xB1f9eE64333564050964241688899166307d446e`, recipient, canonical USDC token, exact amount, `paymentReference = ARK-20260913-A1E97613`, and plaintext `memo = Aptos` from the V2 `InvoicePayment` event before the invoice and bound intent became Paid.
- Scan to Pay WalletConnect QR lifecycle is **LOCAL RUNTIME PASS**: a fresh invoice opens QR on the first attempt, closing and reopening Scan shows QR again without creating another Payment Intent, and completing one invoice releases its session so the next invoice opens a new QR without reload.
