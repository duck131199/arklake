...
- Pay with Arklake + plaintext Memo V2 integration is **LOCAL RUNTIME PASS** and has not been tested on production `arklake.site`. Local invoice `ARK-20260912-42FBD2FE` paid 1 USDC through the Circle UCW atomic V2 flow in transaction `0x3ee3133cf942de8eac7533beee23d240d0b53d075cfbb6018038f58f456f4865`; strict verification matched the canonical USDC payer, recipient, token and amount plus the V2 `InvoicePayment` event with exact `paymentReference = ARK-20260912-42FBD2FE` and plaintext `memo = Thank Duck` before the atomic Paid transition. Arcscan InvoicePayment decoding PASS.
